...
interface EquipmentDeleteModalProps {
  ...
  itemCode: string;
  isDeleting: boolean;
  type: 'Equipment' | 'Bundle Item';
}

const EquipmentDeleteModal: React.FC<EquipmentDeleteModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  title,
  ...
  const isBulk = itemName.includes('items');
      ...
      <div className="relative bg-white w-full max-w-[420px] rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-10 flex flex-col items-center text-center">
          ...
          <h3 className="text-2xl font-black text-slate-800 tracking-tight mb-2">
            Delete {isBulk ? `${type}s` : type}?
          </h3>
          <p className="text-slate-500 text-sm font-medium leading-relaxed mb-8 px-2">
            {isBulk ? (
              <>Are you sure you want to remove <span className="font-black text-slate-800">{itemName}</span> from the database?</>
            ) : (
              <>Are you sure you want to remove <span className="font-black text-slate-800">{itemName}</span> (<span className="font-mono text-xs text-[#FE4E02] font-bold">{itemCode}</span>)?</>
            )}
            <br />
